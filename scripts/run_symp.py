'''
Sensitivity for symptomaticity
'''
...
if __name__ == '__main__':
    ...
    sweep_pars = dict(alt_symp=[False, True])
    huevar = 'AltSymp'
    ...
    if not args.outbreak:
        ...
        # Create and run
        mgr = sct.Manager(name='Symptomaticity', sweep_pars=sweep_pars, sim_pars=sim_pars, levels=None)
        mgr.run(args.force)
        ...
        # Create and run
        mgr = sct.Manager(name='OutbreakSymp', sweep_pars=sweep_pars, sim_pars=sim_pars, levels=levels)
        mgr.run(args.force)
        analyzer = mgr.analyze()
